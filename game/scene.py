...
class BaseScene:
    ...
    def move_base(self):
        ...
        self.display.blit(
            self.background_base, (self.base_pos_x + (336 * SCALE), self.base_pos_y)
        )

    def show_score(self):
        if BaseScene.score < 99:
            if BaseScene.score <= 9:
                self.numbers_rect = self.numbers[0].get_rect(
                    center=(self.display.get_width() / 2, 50 * SCALE)
                )
                self.display.blit(self.numbers[int(BaseScene.score)], self.numbers_rect)
            else:
                self.numbers_rect = self.numbers[0].get_rect(
                    center=(self.display.get_width() / 2 - (12 * SCALE), 50 * SCALE)
                )
                self.display.blit(
                    self.numbers[int(BaseScene.score / 10)], self.numbers_rect
                )
                self.numbers_rect = self.numbers[0].get_rect(
                    center=(self.display.get_width() / 2 + (12 * SCALE), 50 * SCALE)
                )
                self.display.blit(
                    self.numbers[int(BaseScene.score - int(BaseScene.score / 10) * 10)],
                    self.numbers_rect,
                )
        else:
            self.numbers_rect = self.numbers[0].get_rect(
...
class MainGame(BaseScene):
            ...
            if event.type == self.pipe_timer:
                ...
                self.pipe_group.add(
                    Pipe(
                        int(self.speed),
                        rotate=True,
                        xy=(self.last_pipe.x, self.last_pipe.y),
                        night=BaseScene.night,
                    )
                )
    ...
    def collision(self):
        if (
            pygame.sprite.spritecollide(self.player.sprite, self.pipe_group, False)
            or self.player.sprite.rect.bottom >= 425 * SCALE
        ):
            Player.player_y = self.player.sprite.rect.y
            self.hit_sound.play()
            if Player.player_y < 400 * SCALE:
...
class GameOver(BaseScene):
    ...
    def run(self):
        ...
        self.display.blit(
            self.game_over,
            self.game_over.get_rect(center=(self.display.get_width() / 2, 100 * SCALE)),
        )
